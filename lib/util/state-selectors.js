...
const getSubmissionsForSelectedInstance = createSelector(
  getBase,
  getSelectedInstanceName,
  (base = Map(), instanceName) => base.getIn(['submissions', instanceName])
);
...
const getSubmission = (state, submitId) => {
  const base = getBase(state);
  if (base) {
    const submissions = getSubmissionsForSelectedInstance(state);
    if (submissions) {
      return submissions[submitId];
    }
  }
  return {};
};
...
const getSubmitStatus = (state, submitId) => getSubmission(state, submitId).status;

const getBuildLogMessages = (state, buildId) => getBuild(state, buildId).logMessages;

const getSubmitLogMessages = (state, submitId) => getSubmission(state, submitId).logMessages;
...
const getStreamsBuildRestUrl = createSelector(
  ...
  (icp4dUrlString, useIcp4dMasterNodeHost, buildRestUrlString) => {
    if (buildRestUrlString && buildRestUrlString.startsWith('/')) {
      return `${icp4dUrlString}${buildRestUrlString}`;
    }
    return useIcp4dMasterNodeHost ? convertUrl(icp4dUrlString, buildRestUrlString) : buildRestUrlString;
  }
);
...
const getStreamsToolkitRestUrl = createSelector(
  ...
  (icp4dUrlString, useIcp4dMasterNodeHost, toolkitRestUrlString) => {
    if (toolkitRestUrlString && toolkitRestUrlString.startsWith('/')) {
      return `${icp4dUrlString}${toolkitRestUrlString}`;
    }
    return useIcp4dMasterNodeHost ? convertUrl(icp4dUrlString, toolkitRestUrlString) : toolkitRestUrlString;
  }
);
...
const getStreamsRestUrl = createSelector(
  ...
  (icp4dUrlString, useIcp4dMasterNodeHost, streamsRestUrlString) => {
    if (streamsRestUrlString && streamsRestUrlString.startsWith('/')) {
      return `${icp4dUrlString}${streamsRestUrlString}`;
    }
...
const getStreamsConsoleUrl = createSelector(
  ...
  (icp4dUrlString, useIcp4dMasterNodeHost, streamsConsoleUrlString) => {
    if (streamsConsoleUrlString && streamsConsoleUrlString.startsWith('/')) {
      return `${icp4dUrlString}${streamsConsoleUrlString}`;
    }
...
const getStreamsJmxUrl = createSelector(
  ...
  (icp4dUrlString, useIcp4dMasterNodeHost, streamsJmxUrlString) => {
    if (streamsJmxUrlString && streamsJmxUrlString.startsWith('/')) {
      return `${icp4dUrlString}${streamsJmxUrlString}`;
    }
