...
import {
  buildV5Selectors as BuildV5Selectors,
  actionTypes,
...
const buildAppEpic = (action, state) => action.pipe(
  ofType(actionTypes.NEW_BUILD),
  withLatestFrom(state),
  mergeMap(([a, s]) => StreamsRestUtils.build.create(s, { originator: IdeSelectors.getBuildOriginator(s), name: action.sourceArchive }).pipe(
    map(createBuildResponse => {
      ...
      const newBuild = BuildV5Selectors.getNewBuild(s, buildId);
      return uploadSource(
        buildId,
        newBuild.get('appRoot'),
        newBuild.get('toolkitRootPath'),
        newBuild.get('fqn'),
        newBuild.get('makefilePath')
      );
    }),
...
const uploadSourceEpic = (action, state) => action.pipe(
  ofType(actionTypes.BUILD_UPLOAD_SOURCE),
  withLatestFrom(state),
  mergeMap(([uploadAction, s]) => SourceArchiveUtils.buildSourceArchive({
    ...
    toolkitCacheDir: IdeSelectors.getToolkitsCacheDir(s),
...
const sourceArchiveCreatedEpic = (action, state) => {
  return action.pipe(
    ofType(actionTypes.SOURCE_ARCHIVE_CREATED),
    withLatestFrom(state),
...
const startBuildEpic = (action, state) => {
  return action.pipe(
    ofType(actionTypes.START_BUILD),
    withLatestFrom(state),
...
const buildStatusEpic = (action, state) => action.pipe(
  ofType(actionTypes.GET_BUILD_STATUS),
  withLatestFrom(state),
...
const buildStatusLoopEpic = (action, state) => action.pipe(
  ofType(actionTypes.BUILD_STATUS_RECEIVED),
  withLatestFrom(state),
  tap(([a, s]) => {
...
const shouldGetBuildStatusHelperObservable = (action, state) => {
  const buildStatus = BuildV5Selectors.getBuildStatus(state, action.buildId);
...
const getBuildArtifactsEpic = (action, state) => action.pipe(
  ofType(actionTypes.GET_BUILD_ARTIFACTS),
  withLatestFrom(state),
...
const getBuildArtifactsFulfilledEpic = (action, state) => action.pipe(
  ofType(actionTypes.GET_BUILD_ARTIFACTS_FULFILLED),
  withLatestFrom(state),
  tap(([a, s]) => {
    const { buildId } = a;
    StatusUtils.downloadOrSubmit(s, buildId);
  }),
  map(() => ({ type: actionTypes.POST_GET_BUILD_ARTIFACTS_FULFILLED })), catchError(error => of(handleError(action, error)))
);

const downloadArtifactsEpic = (action, state) => action.pipe(
  ofType(actionTypes.DOWNLOAD_BUILD_ARTIFACTS),
  withLatestFrom(state),
  mergeMap(([a, s]) => {
    const { buildId } = a;
    ...
    return from(artifacts).pipe(
      mergeMap(artifact => StreamsRestUtils.artifact.downloadApplicationBundle(s, buildId, artifact.id).pipe(
        map(downloadResponse => {
          ...
          const artifactOutputPath = BuildV5Selectors.getOutputArtifactFilePath(s, buildId, artifactId);
          try {
      ...
      map(() => ({ type: actionTypes.POST_DOWNLOAD_BUILD_ARTIFACTS })),
      catchError(error => of(handleError(a, error)))
    );
  }), catchError(error => of(handleError(action, error)))
);
...
const refreshToolkitsEpic = (action, state) => action.pipe(
  ofType(actionTypes.REFRESH_TOOLKITS),
  withLatestFrom(state),
  mergeMap(([a, s]) => StreamsRestUtils.toolkit.getToolkits(s).pipe(
    ...
    map(() => ({ type: actionTypes.POST_REFRESH_TOOLKITS })),
