...
import { some as _some, intersectionWith as _intersectionWith } from 'lodash';
import * as xmldoc from 'xmldoc';
import * as semver from 'semver';

import { from } from 'rxjs';
import { actionTypes } from '../build/v5';
import { MessageHandlerRegistry, StreamsToolkitsUtils } from '.';
...
async function buildSourceArchive(
  ...
  try {
    ...
    let tkPathString = '';
    if (toolkitPaths && toolkitPaths.length > 0) {
      ...
      rootContents
        ...
        .filter(item => !_some(ignoreFiles, name => {
          if (name.includes('*')) {
      ...
      rootContents
        ...
        .filter(item => !_some(ignoreDirs, name => {
          if (name.includes('*')) {
            const regex = new RegExp(name.replace('.', '\.').replace('*', '.*'));
            return regex.test(item);
          }
    ...
    return { type: actionTypes.SOURCE_ARCHIVE_CREATED, archivePath: outputFilePath, buildId };
  } catch (err) {
    ...
    try {
      ...
      if (dependenciesNode) {
        ...
        if (dependencyToolkitsNodes) {
          ...
          const toolkitsToInclude = _intersectionWith(newestLocalToolkits, parsedDependencies, (tk, dependency) => tk.name === dependency.name && semver.satisfies(semver.coerce(tk.version), dependency.version));
