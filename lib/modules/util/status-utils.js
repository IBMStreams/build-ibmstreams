...
import { buildV5Selectors as BuildV5Selectors, downloadBuildArtifacts } from '../build/v5';
import { submitV5Selectors as SubmitV5Selectors, getSubmissionTimeParamsFromAdl } from '../submit/v5';
import { connectionV5Selectors as ConnV5Selectors } from '../connection/v5';
import { openStreamsConsole } from '../ide';

function buildStatusUpdate(action, state) {
  const { buildId } = action;
  const buildStatus = BuildV5Selectors.getBuildStatus(state, buildId);
  const logMessages = BuildV5Selectors.getBuildLogMessages(state, buildId);
  const displayIdentifier = BuildV5Selectors.getBuildDisplayIdentifier(state, buildId);
...
function submitStatusUpdate(action, state) {
  const { id } = action;
  const submitStatus = SubmitV5Selectors.getSubmitStatus(state, id);
  const logMessages = SubmitV5Selectors.getSubmitLogMessages(state, id);
  const displayIdentifier = action.identifier;
  ...
  } else if (submitStatus === 'job.applicationBundleUploading') {
    messageHandler.handleInfo(`application bundle uploading - ${displayIdentifier}...`, { detail: logMessages.join('\n'), showNotification: true });
  }
}
...
function downloadOrSubmit(state, buildId) {
  const buildStatus = BuildV5Selectors.getBuildStatus(state, buildId);
  if (buildStatus === 'built') {
    const artifacts = BuildV5Selectors.getBuildArtifacts(state, buildId);
    const messageHandler = getMessageHandlerForBuildId(state, buildId);
    const identifier = BuildV5Selectors.getMessageHandlerIdentifier(state, buildId);
    const postBuildAction = BuildV5Selectors.getPostBuildAction(state, buildId);

    if (BUILD_ACTION.SUBMIT === postBuildAction) {
          ...
          notificationButtons: [
            {
              ...
              callbackFn: () => {
                ...
                getStore().dispatch(getSubmissionTimeParamsFromAdl(buildId, true, identifier));
              }
            },
            {
              ...
              callbackFn: () => {
                ...
                getStore().dispatch(downloadBuildArtifacts(buildId));
                getStore().dispatch(openStreamsConsole());
              }
            }
          ]
        });
...
function jobSubmitted(state, submitInfo, buildId) {
  ...
  if (submitInfo.status === 'running') {
    messageHandler.handleSuccess(
      `Job ${submitInfo.name} has been successfully submitted to the ${ConnV5Selectors.getSelectedInstanceName(state)} instance`,
      {
        ...
        notificationButtons: [
          {
            ...
            callbackFn: () => {
              const icp4dUrlStr = ConnV5Selectors.getIcp4dUrl(state);
              const icp4dUrl = new URL(icp4dUrlStr); /* eslint-disable-line compat/compat */
              const icp4dUrlBase = `${icp4dUrl.protocol}//${icp4dUrl.host}`;
              const jobDetailsUrl = `${icp4dUrlBase}/streams/webpage/#/streamsJobDetails/streams-${ConnV5Selectors.getServiceInstanceId(state)}-${submitInfo.id}`;
          ...
          },
          {
            ...
            callbackFn: () => {
              const consoleUrl = ConnV5Selectors.getStreamsConsoleUrl(state);
...
function getLintHandlerForBuildId(state, buildId) {
  const appRoot = BuildV5Selectors.getBuildAppRoot(state, buildId);
  return LintHandlerRegistry.get(appRoot);
}

function getMessageHandlerForSubmitId(state, submitId) {
  // TODO: create a message handler for the job submission
  return MessageHandlerRegistry.getDefault();
}
