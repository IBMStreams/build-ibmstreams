...
import { difference as _difference, uniqWith as _uniqWith } from 'lodash';
import * as semver from 'semver';
import * as xmldoc from 'xmldoc';
import { ideSelectors as IdeSelectors } from '../ide';
...
function refreshLspToolkits(state, sendNotification) {
  ...
  const addParam = getLangServerParamForAddToolkits([
    ...getCachedToolkitIndexPaths(IdeSelectors.getToolkitsCacheDir(state)),
    ...getLocalToolkitIndexPaths(IdeSelectors.getToolkitsPathSetting(state))
  ]);
...
function getChangedLocalToolkits(oldValue, newValue) {
  ...
  const addedToolkitPaths = _difference(newIndexPaths, oldIndexPaths);
  const removedToolkitPaths = _difference(oldIndexPaths, newIndexPaths);
...
function getToolkitsToCache(state, buildServiceToolkits) {
  const cacheDir = IdeSelectors.getToolkitsCacheDir(state);
  if (!cacheDir) {
...
function filterNewestToolkits(toolkitList) {
  ...
  return _uniqWith(toolkitList, (tkA, tkB) => tkA.name === tkB.name && semver.gt(tkA.version, tkB.version));
}
