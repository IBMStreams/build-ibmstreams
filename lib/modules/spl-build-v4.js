...
import { map as _map, findIndex as _findIndex, some as _some } from 'lodash';
...
import { StreamsUtils, BUILD_ACTION } from '.';
...
export class SplBuilder {
  ...
  build(action, streamingAnalyticsCredentials, input) {
    ...
    if (this.serviceCredentials.apikey && this.serviceCredentials.v2_rest_url) {
      if (BUILD_ACTION.DOWNLOAD === action) {
        this.buildAndDownloadBundle(input);
      } else if (BUILD_ACTION.SUBMIT === action) {
    ...
    const dialogButtons = [
      {
        ...
        callbackFn: () => {
          ...
          if (submissionObservableInput.artifacts) {
            this.downloadBundlesObservable(submissionObservableInput).pipe(
              map(downloadOutput => ([submissionObservableInput, downloadOutput])),
              mergeMap(downloadResult => this.performBundleDownloads(downloadResult, null, `${outputDir}${path.sep}temp`)),
            ).subscribe(
              next => { },
              err => {
                let errorNotification = null;
              ...
              complete => {
                StreamsUtils.extractSab(`${outputDir}${path.sep}temp${path.sep}${submissionObservableInput.artifacts[0].name}`, {
                  type: 'V4', submissionObservableFunc, submissionObservableInput, submitFunc
                }).then(panel => {
                  if (panel) {
                    panel.show();
                  } else {
                    submitFunc(submissionObservableFunc, submissionObservableInput);
                  }
                  fse.removeSync(`${outputDir}${path.sep}temp`);
                });
              }
            );
          } else {
            StreamsUtils.extractSab(submissionObservableInput.filename, {
              type: 'V4', submissionObservableFunc, submissionObservableInput, submitFunc
            }).then(panel => {
              if (panel) {
                panel.show();
  ...
  downloadBundlesObservable(input) {
    ...
    const observables = _map(input.artifacts, artifact => {
  ...
  submitSabObservable(input) {
    ...
    const jobConfig = `{"jobConfigOverlays": [{"jobConfig": {"submissionParameters": ${input.submissionTimeValues}}}]}`;
  ...
  performBundleDownloads(downloadResult, input, outputDirOverride = undefined) {
    ...
    const observables = _map(statusOutput.artifacts, artifact => {
      const index = _findIndex(statusOutput.artifacts, artifactObj => artifactObj.name === artifact.name);
  ...
  static getApplicationRoot(rootDirArray, filePath) {
    if (typeof (filePath) === 'string' && Array.isArray(rootDirArray)) {
      ...
      const notWorkspaceFolder = dir => (
        !_some(rootDirArray, folder => folder === dir)
      );
...
const SplBuildCommonV4 = {
  setTimeout
};

export default SplBuildCommonV4;
