...
const streamsV5Build = (state = [], action) => {
  ...
  switch (action.type) {
    ...
    case actions.SET_SELECTED_INSTANCE:
      return {
        ...
        selectedInstance: {
          ...
          // for toolkit endpoint, can be separate or subpart of the build service endpoint
          streamsToolkitRestUrl: action.CreateArguments['connection-info'].externalBuildToolkitEndpoint
            || (action.CreateArguments['connection-info'].externalBuildEndpoint
              && action.CreateArguments['connection-info'].externalBuildEndpoint.endsWith('/builds')
              ? `${action.CreateArguments['connection-info'].externalBuildEndpoint.substring(0, action.CreateArguments['connection-info'].externalBuildEndpoint.lastIndexOf('/builds'))}/toolkits`
              : `${action.CreateArguments['connection-info'].externalBuildEndpoint}/toolkits`),
    ...
    case actions.GET_SUBMIT_STATUS_FULFILLED:
      return {
        ...state,
        submissions: {
          ...state.submissions,
          [state.selectedInstance.instanceName]: {
            ...(state.submissions && state.submissions[state.selectedInstance.instanceName]),
            [action.id]: {
              ...state.submissions[state.selectedInstance.instanceName][action.id],
              status: action.status,
            }
          }
        }
      };
    case actions.SUBMIT_APPLICATIONS_FROM_BUNDLE_FILES_ASYNC:
      return {
        ...state,
        submissions: {
          ...state.submissions,
    ...
    case actions.ASYNC_SUBMIT_APPLICATIONS:
      return {
        ...state,
        submissions: {
          ...state.submissions,
    ...
    case actions.GET_SUBMIT_LOG_MESSAGES_FULFILLED:
      return {
        ...state,
        submissions: {
          ...state.submissions,
