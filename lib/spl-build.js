...
import SplBuildCommonV4, { SplBuilder } from './spl-build-common';
...
const CONF_REST_URL = 'build-ibmstreams.restServiceUrl';
const CONF_BUILD_URL = 'build-ibmstreams.buildServiceUrl';
const CONF_API_VERSION = 'build-ibmstreams.buildApiVersion';
const CONF_API_VERSION_V4 = 'v4';
const CONF_API_VERSION_V5_CP4D = 'v5';
const CONF_API_VERSION_V5_STANDALONE = 'v5Standalone';
...
function updateUrlInStore(urlString, actionCreator) {
  try {
    ...
    getStore().dispatch(actionCreator(null));
  }
}
...
export default {
  ...
  config: {
    buildApiVersion: {
      ...
      enum: [
        ...
        { value: CONF_API_VERSION_V5_CP4D, description: 'IBM Cloud Pak for Data Streams Add-on' },
        { value: CONF_API_VERSION_V5_STANDALONE, description: 'IBM Streams Standalone' }
      ]
    },
    requestTimeout: {
  ...
  activate(state) {
    ...
    this.subscriptions.add(
      atom.config.onDidChange(CONF_REST_URL, {}, (event) => {
        try {
          ...
          getStore().dispatch(resetAuth());
          const parsedUrl = new URL(event.newValue); // eslint-disable-line compat/compat
          updateUrlInStore(parsedUrl, setStreamsRestUrl);
        } catch (err) { /* do nothing */ }
      })
    );
    ...
    if (!StateSelector.getStreamsBuildRestUrl(getStore().getState())) {
      updateUrlInStore(atom.config.get(CONF_BUILD_URL), setStreamsBuildUrl);
    }
    if (!StateSelector.getStreamsRestUrl(getStore().getState())) {
      updateUrlInStore(atom.config.get(CONF_REST_URL), setStreamsRestUrl);
    }
  ...
  shouldShowMenuV5(event) {
    return this.shouldShowMenu(event) && (this.apiVersion === CONF_API_VERSION_V5_CP4D || this.apiVersion === CONF_API_VERSION_V5_STANDALONE);
  },
  handleBuildCallback(e) {
    if (this.atomIdeUiPackageInstalled) {
      ...
      if (selectedComp) {
        ...
        if (this.apiVersion === CONF_API_VERSION_V5_CP4D || this.apiVersion === CONF_API_VERSION_V5_STANDALONE) {
  ...
  */
  ...
  openIcp4dDashboard() {
    if (this.apiVersion === CONF_API_VERSION_V5_CP4D) {
      const openDashboard = () => {
        try {
